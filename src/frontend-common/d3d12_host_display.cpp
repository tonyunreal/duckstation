...
public:
  ...
  static std::unique_ptr<D3D12HostDisplayTexture> Create(u32 width, u32 height, const void* data, u32 data_stride)
  {
    ...
    if (data && !texture.LoadData(0, 0, width, height, data, data_stride))
      return {};

    return std::make_unique<D3D12HostDisplayTexture>(std::move(texture));
  }
...
{
  return D3D12HostDisplayTexture::Create(width, height, initial_data, initial_data_stride);
}
