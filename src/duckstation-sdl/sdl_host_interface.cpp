...
bool SDLHostInterface::CreateDisplay()
{
  ...
  if (!FullscreenUI::Initialize(this, m_settings_interface.get()))
  {
...
void SDLHostInterface::ApplySettings()
{
  Settings old_settings(std::move(g_settings));
  CommonHostInterface::LoadSettings(*m_settings_interface.get());
...
void SDLHostInterface::DrawQuickSettingsMenu()
{
  ...
  Settings& settings_copy = FullscreenUI::GetSettingsCopy();

  if (ImGui::BeginMenu("CPU Execution Mode"))
  {
    const CPUExecutionMode current = settings_copy.cpu_execution_mode;
    for (u32 i = 0; i < static_cast<u32>(CPUExecutionMode::Count); i++)
    {
      ...
      {
        settings_copy.cpu_execution_mode = static_cast<CPUExecutionMode>(i);
  ...
  if (ImGui::MenuItem("CPU Clock Control", nullptr, &settings_copy.cpu_overclock_enable))
  {
    settings_changed = true;
    settings_copy.UpdateOverclockActive();
  }
  ...
  if (ImGui::BeginMenu("CPU Clock Speed"))
  {
    ...
    const u32 percent = settings_copy.GetCPUOverclockPercent();
    for (u32 value : values)
    {
      if (ImGui::MenuItem(TinyString::FromFormat("%u%%", value), nullptr, percent == value))
      {
        settings_copy.SetCPUOverclockPercent(value);
        settings_copy.UpdateOverclockActive();
        settings_changed = true;
      }
    }
  ...
  settings_changed |=
    ImGui::MenuItem("Recompiler Memory Exceptions", nullptr, &settings_copy.cpu_recompiler_memory_exceptions);
  if (ImGui::BeginMenu("Recompiler Fastmem"))
  {
    for (u32 i = 0; i < static_cast<u32>(CPUFastmemMode::Count); i++)
    {
      if (ImGui::MenuItem(Settings::GetCPUFastmemModeDisplayName(static_cast<CPUFastmemMode>(i)), nullptr,
                          settings_copy.cpu_fastmem_mode == static_cast<CPUFastmemMode>(i)))
      {
        settings_copy.cpu_fastmem_mode = static_cast<CPUFastmemMode>(i);
        settings_changed = true;
      }
  ...
  settings_changed |= ImGui::MenuItem("Recompiler ICache", nullptr, &settings_copy.cpu_recompiler_icache);

  ImGui::Separator();

  if (ImGui::BeginMenu("Renderer"))
  {
    const GPURenderer current = settings_copy.gpu_renderer;
    for (u32 i = 0; i < static_cast<u32>(GPURenderer::Count); i++)
    {
      if (ImGui::MenuItem(Settings::GetRendererDisplayName(static_cast<GPURenderer>(i)), nullptr,
                          i == static_cast<u32>(current)))
      {
        settings_copy.gpu_renderer = static_cast<GPURenderer>(i);
        settings_changed = true;
      }
    }

    settings_changed |= ImGui::MenuItem("GPU on Thread", nullptr, &settings_copy.gpu_use_thread);
  ...
  settings_changed |= ImGui::MenuItem("VSync", nullptr, &settings_copy.video_sync_enabled);

  ImGui::Separator();

  if (ImGui::BeginMenu("Resolution Scale"))
  {
    const u32 current_internal_resolution = settings_copy.gpu_resolution_scale;
    for (u32 scale = 1; scale <= GPU::MAX_RESOLUTION_SCALE; scale++)
    {
      ...
      {
        settings_copy.gpu_resolution_scale = scale;
        settings_changed = true;
      }
    }
  ...
  {
    const u32 current_multisamples = settings_copy.gpu_multisamples;
    const bool current_ssaa = settings_copy.gpu_per_sample_shading;

    if (ImGui::MenuItem("None", nullptr, (current_multisamples == 1)))
    {
      settings_copy.gpu_multisamples = 1;
      settings_copy.gpu_per_sample_shading = false;
      settings_changed = true;
    }
    ...
    for (u32 i = 2; i <= 32; i *= 2)
    {
      ...
      {
        settings_copy.gpu_multisamples = i;
        settings_copy.gpu_per_sample_shading = false;
        settings_changed = true;
      }
    }
    ...
    for (u32 i = 2; i <= 32; i *= 2)
    {
      ...
      {
        settings_copy.gpu_multisamples = i;
        settings_copy.gpu_per_sample_shading = true;
        settings_changed = true;
      }
    }
  ...
  {
    settings_changed |= ImGui::MenuItem("PGXP Enabled", nullptr, &settings_copy.gpu_pgxp_enable);
    settings_changed |=
      ImGui::MenuItem("PGXP Culling", nullptr, &settings_copy.gpu_pgxp_culling, settings_copy.gpu_pgxp_enable);
    settings_changed |= ImGui::MenuItem("PGXP Texture Correction", nullptr,
                                        &settings_copy.gpu_pgxp_texture_correction, settings_copy.gpu_pgxp_enable);
    settings_changed |= ImGui::MenuItem("PGXP Vertex Cache", nullptr, &settings_copy.gpu_pgxp_vertex_cache,
                                        settings_copy.gpu_pgxp_enable);
    settings_changed |=
      ImGui::MenuItem("PGXP CPU Instructions", nullptr, &settings_copy.gpu_pgxp_cpu, settings_copy.gpu_pgxp_enable);
    settings_changed |= ImGui::MenuItem("PGXP Preserve Projection Precision", nullptr,
                                        &settings_copy.gpu_pgxp_preserve_proj_fp, settings_copy.gpu_pgxp_enable);
    settings_changed |= ImGui::MenuItem("PGXP Depth Buffer", nullptr, &settings_copy.gpu_pgxp_depth_buffer,
                                        settings_copy.gpu_pgxp_enable);
    ImGui::EndMenu();
  }

  settings_changed |= ImGui::MenuItem("True (24-Bit) Color", nullptr, &settings_copy.gpu_true_color);
  settings_changed |= ImGui::MenuItem("Scaled Dithering", nullptr, &settings_copy.gpu_scaled_dithering);

  if (ImGui::BeginMenu("Texture Filtering"))
  {
    const GPUTextureFilter current = settings_copy.gpu_texture_filter;
    for (u32 i = 0; i < static_cast<u32>(GPUTextureFilter::Count); i++)
    {
      if (ImGui::MenuItem(Settings::GetTextureFilterDisplayName(static_cast<GPUTextureFilter>(i)), nullptr,
                          i == static_cast<u32>(current)))
      {
        settings_copy.gpu_texture_filter = static_cast<GPUTextureFilter>(i);
        settings_changed = true;
      }
    }
  ...
  settings_changed |= ImGui::MenuItem("Disable Interlacing", nullptr, &settings_copy.gpu_disable_interlacing);
  settings_changed |= ImGui::MenuItem("Widescreen Hack", nullptr, &settings_copy.gpu_widescreen_hack);
  settings_changed |= ImGui::MenuItem("Force NTSC Timings", nullptr, &settings_copy.gpu_force_ntsc_timings);
  settings_changed |= ImGui::MenuItem("24-Bit Chroma Smoothing", nullptr, &settings_copy.gpu_24bit_chroma_smoothing);

  ImGui::Separator();

  settings_changed |= ImGui::MenuItem("Display Linear Filtering", nullptr, &settings_copy.display_linear_filtering);
  settings_changed |= ImGui::MenuItem("Display Integer Scaling", nullptr, &settings_copy.display_integer_scaling);
  ...
  if (ImGui::BeginMenu("Aspect Ratio"))
  {
    for (u32 i = 0; i < static_cast<u32>(DisplayAspectRatio::Count); i++)
    {
      if (ImGui::MenuItem(Settings::GetDisplayAspectRatioName(static_cast<DisplayAspectRatio>(i)), nullptr,
                          settings_copy.display_aspect_ratio == static_cast<DisplayAspectRatio>(i)))
      {
        settings_copy.display_aspect_ratio = static_cast<DisplayAspectRatio>(i);
        settings_changed = true;
      }
  ...
  if (ImGui::BeginMenu("Crop Mode"))
  {
    for (u32 i = 0; i < static_cast<u32>(DisplayCropMode::Count); i++)
    {
      if (ImGui::MenuItem(Settings::GetDisplayCropModeDisplayName(static_cast<DisplayCropMode>(i)), nullptr,
                          settings_copy.display_crop_mode == static_cast<DisplayCropMode>(i)))
      {
        settings_copy.display_crop_mode = static_cast<DisplayCropMode>(i);
        settings_changed = true;
      }
  ...
  if (ImGui::BeginMenu("Downsample Mode"))
  {
    for (u32 i = 0; i < static_cast<u32>(GPUDownsampleMode::Count); i++)
    {
      if (ImGui::MenuItem(Settings::GetDownsampleModeDisplayName(static_cast<GPUDownsampleMode>(i)), nullptr,
                          settings_copy.gpu_downsample_mode == static_cast<GPUDownsampleMode>(i)))
      {
        settings_copy.gpu_downsample_mode = static_cast<GPUDownsampleMode>(i);
        settings_changed = true;
      }
  ...
  settings_changed |= ImGui::MenuItem("Force 4:3 For 24-bit", nullptr, &settings_copy.display_force_4_3_for_24bit);
  ...
  if (settings_changed)
    RunLater(FullscreenUI::SaveAndApplySettings);
}
...
void SDLHostInterface::DrawDebugMenu()
{
  ...
  if (ImGui::BeginMenu("Log Level"))
  {
    for (u32 i = LOGLEVEL_NONE; i < LOGLEVEL_COUNT; i++)
    {
      ...
      {
        settings_copy.log_level = static_cast<LOGLEVEL>(i);
        settings_changed = true;
      }
    }
  ...
  settings_changed |= ImGui::MenuItem("Log To Console", nullptr, &settings_copy.log_to_console);
  settings_changed |= ImGui::MenuItem("Log To Debug", nullptr, &settings_copy.log_to_debug);
  settings_changed |= ImGui::MenuItem("Log To File", nullptr, &settings_copy.log_to_file);

  ImGui::Separator();

  settings_changed |= ImGui::MenuItem("Disable All Enhancements", nullptr, &settings_copy.disable_all_enhancements);
  ...
  if (settings_changed)
  {
    ...
    Settings::DebugSettings& debug_settings_copy = settings_copy.debugging;
  ...
  if (ImGui::BeginTabBar("SettingsTabBar", 0))
  {
    ...
    if (ImGui::BeginTabItem("General"))
    {
      if (DrawSettingsSectionHeader("Console"))
      {
        ...
        int region = static_cast<int>(settings_copy.region);
        if (ImGui::Combo(
              "##region", &region,
        ...
        {
          settings_copy.region = static_cast<ConsoleRegion>(region);
          settings_changed = true;
        }

        settings_changed |= ImGui::Checkbox("Enable TTY Output", &settings_copy.bios_patch_tty_enable);
        settings_changed |= ImGui::Checkbox("Fast Boot", &settings_copy.bios_patch_fast_boot);
      }
      ...
      if (DrawSettingsSectionHeader("Behavior"))
      {
        ...
        settings_changed |= ImGui::SliderFloat("##speed", &settings_copy.emulation_speed, 0.25f, 5.0f);
        settings_changed |= ImGui::Checkbox("Increase Timer Resolution", &settings_copy.increase_timer_resolution);
        settings_changed |= ImGui::Checkbox("Pause On Start", &settings_copy.start_paused);
        settings_changed |= ImGui::Checkbox("Start Fullscreen", &settings_copy.start_fullscreen);
        settings_changed |= ImGui::Checkbox("Save State On Exit", &settings_copy.save_state_on_exit);
        settings_changed |= ImGui::Checkbox("Apply Game Settings", &settings_copy.apply_game_settings);
        settings_changed |= ImGui::Checkbox("Automatically Load Cheats", &settings_copy.auto_load_cheats);
        settings_changed |=
          ImGui::Checkbox("Load Devices From Save States", &settings_copy.load_devices_from_save_states);
      }

      ImGui::NewLine();
      if (DrawSettingsSectionHeader("CDROM Emulation"))
      {
        settings_changed |= ImGui::Checkbox("Use Read Thread (Asynchronous)", &settings_copy.cdrom_read_thread);
        settings_changed |= ImGui::Checkbox("Enable Region Check", &settings_copy.cdrom_region_check);
        settings_changed |= ImGui::Checkbox("Preload Image To RAM", &settings_copy.cdrom_load_image_to_ram);
      }
      ...
      if (DrawSettingsSectionHeader("Audio"))
      {
        ...
        int backend = static_cast<int>(settings_copy.audio_backend);
        if (ImGui::Combo(
              "##backend", &backend,
              [](void*, int index, const char** out_text) {
        ...
        {
          settings_copy.audio_backend = static_cast<AudioBackend>(backend);
          settings_changed = true;
        }

        settings_changed |= ImGui::Checkbox("Output Sync", &settings_copy.audio_sync_enabled);
        settings_changed |= ImGui::Checkbox("Start Dumping On Boot", &settings_copy.audio_dump_on_boot);
        settings_changed |= ImGui::Checkbox("Mute CD Audio", &settings_copy.cdrom_mute_cd_audio);
      }
    ...
    if (ImGui::BeginTabItem("Ports"))
    {
      for (int i = 0; i < 2; i++)
      {
        ...
        if (DrawSettingsSectionHeader(buf))
        {
          ...
          int controller_type = static_cast<int>(settings_copy.controller_types[i]);
          if (ImGui::Combo(
          ...
          {
            settings_copy.controller_types[i] = static_cast<ControllerType>(controller_type);
            settings_changed = true;
          }
          ...
          int memory_card_type = static_cast<int>(settings_copy.memory_card_types[i]);
          if (ImGui::Combo(
          ...
          {
            settings_copy.memory_card_types[i] = static_cast<MemoryCardType>(memory_card_type);
            settings_changed = true;
          }
          ...
          std::string* path_ptr = &settings_copy.memory_card_paths[i];
    ...
    if (ImGui::BeginTabItem("CPU"))
    {
      ...
      int execution_mode = static_cast<int>(settings_copy.cpu_execution_mode);
      if (ImGui::Combo(
      ...
      {
        settings_copy.cpu_execution_mode = static_cast<CPUExecutionMode>(execution_mode);
        settings_changed = true;
      }

      settings_changed |= ImGui::Checkbox("Enable CPU Clock Control", &settings_copy.cpu_overclock_enable);
      if (settings_copy.cpu_overclock_enable)
      {
        ImGui::Text("Overclock:");
        ImGui::SameLine(indent);

        int overclock_percent = static_cast<int>(settings_copy.GetCPUOverclockPercent());
        if (ImGui::SliderInt("##overclock_percent", &overclock_percent, 1, 1000, "%d%%"))
        {
          settings_copy.SetCPUOverclockPercent(static_cast<u32>(overclock_percent));
          settings_changed = true;
        }
      }

      settings_changed |=
        ImGui::Checkbox("Enable Recompiler Memory Exceptions", &settings_copy.cpu_recompiler_memory_exceptions);

      settings_changed |= ImGui::Checkbox("Enable Recompiler ICache", &settings_copy.cpu_recompiler_icache);
    ...
    if (ImGui::BeginTabItem("GPU"))
    {
      if (DrawSettingsSectionHeader("Basic"))
      {
        ...
        int gpu_renderer = static_cast<int>(settings_copy.gpu_renderer);
        if (ImGui::Combo(
        ...
        {
          settings_copy.gpu_renderer = static_cast<GPURenderer>(gpu_renderer);
          settings_changed = true;
        }
      }
      ...
      if (DrawSettingsSectionHeader("Display Output"))
      {
        ...
        ImGui::SameLine(indent);
        int display_aspect_ratio = static_cast<int>(settings_copy.display_aspect_ratio);
        if (ImGui::Combo(
        ...
        {
          settings_copy.display_aspect_ratio = static_cast<DisplayAspectRatio>(display_aspect_ratio);
          settings_changed = true;
        }
        ...
        int display_crop_mode = static_cast<int>(settings_copy.display_crop_mode);
        if (ImGui::Combo(
        ...
        {
          settings_copy.display_crop_mode = static_cast<DisplayCropMode>(display_crop_mode);
          settings_changed = true;
        }

        ImGui::Text("Downsample Mode:");
        ImGui::SameLine(indent);
        int gpu_downsample_mode = static_cast<int>(settings_copy.gpu_downsample_mode);
        if (ImGui::Combo(
        ...
        {
          settings_copy.gpu_downsample_mode = static_cast<GPUDownsampleMode>(gpu_downsample_mode);
          settings_changed = true;
        }

        settings_changed |= ImGui::Checkbox("Use Debug Device", &settings_copy.gpu_use_debug_device);
        settings_changed |= ImGui::Checkbox("Linear Filtering", &settings_copy.display_linear_filtering);
        settings_changed |= ImGui::Checkbox("Integer Scaling", &settings_copy.display_integer_scaling);
        settings_changed |= ImGui::Checkbox("VSync", &settings_copy.video_sync_enabled);
      }
      ...
      if (DrawSettingsSectionHeader("Enhancements"))
      {
        ...
        int current_resolution_index = static_cast<int>(settings_copy.gpu_resolution_scale) - 1;
        if (ImGui::Combo("##gpu_resolution_scale", &current_resolution_index, resolutions.data(),
                         static_cast<int>(resolutions.size())))
        {
          settings_copy.gpu_resolution_scale = static_cast<u32>(current_resolution_index + 1);
          settings_changed = true;
        }

        ImGui::Text("Texture Filtering:");
        ImGui::SameLine(indent);
        int gpu_texture_filter = static_cast<int>(settings_copy.gpu_texture_filter);
        if (ImGui::Combo(
        ...
        {
          settings_copy.gpu_texture_filter = static_cast<GPUTextureFilter>(gpu_texture_filter);
          settings_changed = true;
        }

        settings_changed |= ImGui::Checkbox("True 24-bit Color (disables dithering)", &settings_copy.gpu_true_color);
        settings_changed |= ImGui::Checkbox("Disable Interlacing", &settings_copy.gpu_disable_interlacing);
        settings_changed |= ImGui::Checkbox("Force NTSC Timings", &settings_copy.gpu_force_ntsc_timings);
        settings_changed |= ImGui::Checkbox("Widescreen Hack", &settings_copy.gpu_widescreen_hack);
        settings_changed |=
          ImGui::Checkbox("Force 4:3 For 24-Bit Display", &settings_copy.display_force_4_3_for_24bit);
        settings_changed |= ImGui::Checkbox("24-Bit Chroma Smoothing", &settings_copy.gpu_24bit_chroma_smoothing);

        settings_changed |= ImGui::Checkbox("PGXP Enabled", &settings_copy.gpu_pgxp_enable);
        settings_changed |= ImGui::Checkbox("PGXP Culling", &settings_copy.gpu_pgxp_culling);
        settings_changed |= ImGui::Checkbox("PGXP Texture Correction", &settings_copy.gpu_pgxp_texture_correction);
        settings_changed |= ImGui::Checkbox("PGXP Vertex Cache", &settings_copy.gpu_pgxp_vertex_cache);
        settings_changed |= ImGui::Checkbox("PGXP CPU Instructions", &settings_copy.gpu_pgxp_cpu);
        settings_changed |= ImGui::Checkbox("PGXP Preserve Projection Precision", &settings_copy.gpu_pgxp_enable);
        settings_changed |= ImGui::Checkbox("PGXP Depth Buffer", &settings_copy.gpu_pgxp_depth_buffer);

        ImGui::Text("PGXP Depth Clear Threshold:");
        ImGui::SameLine(indent);

        float depth_clear_threshold = settings_copy.GetPGXPDepthClearThreshold();
        if (ImGui::SliderFloat("##clear_threshold", &depth_clear_threshold, 0.0f, 4096.0f))
        {
          settings_copy.SetPGXPDepthClearThreshold(depth_clear_threshold);
          settings_changed = true;
        }
      }
    ...
    if (ImGui::BeginTabItem("Advanced"))
    {
      ...
      int dma_max_slice_ticks = static_cast<int>(settings_copy.dma_max_slice_ticks);
      if (ImGui::SliderInt("##dma_max_slice_ticks", &dma_max_slice_ticks, 100, 10000))
      {
        settings_copy.dma_max_slice_ticks = dma_max_slice_ticks;
        settings_changed = true;
      }
      ...
      int dma_halt_ticks = static_cast<int>(settings_copy.dma_halt_ticks);
      if (ImGui::SliderInt("##dma_halt_ticks", &dma_halt_ticks, 100, 10000))
      {
        settings_copy.dma_halt_ticks = dma_halt_ticks;
        settings_changed = true;
      }
      ...
      int gpu_fifo_size = static_cast<int>(settings_copy.gpu_fifo_size);
      if (ImGui::SliderInt("##gpu_fifo_size", &gpu_fifo_size, 16, GPU::MAX_FIFO_SIZE))
      {
        settings_copy.gpu_fifo_size = gpu_fifo_size;
        settings_changed = true;
      }
      ...
      int gpu_max_run_ahead = static_cast<int>(settings_copy.gpu_max_run_ahead);
      if (ImGui::SliderInt("##gpu_max_run_ahead", &gpu_max_run_ahead, 0, 1000))
      {
        settings_copy.gpu_max_run_ahead = gpu_max_run_ahead;
        settings_changed = true;
      }

      if (ImGui::Button("Reset"))
      {
        settings_copy.dma_max_slice_ticks = static_cast<TickCount>(Settings::DEFAULT_DMA_MAX_SLICE_TICKS);
        settings_copy.dma_halt_ticks = static_cast<TickCount>(Settings::DEFAULT_DMA_HALT_TICKS);
        settings_copy.gpu_fifo_size = Settings::DEFAULT_GPU_FIFO_SIZE;
        settings_copy.gpu_max_run_ahead = static_cast<TickCount>(Settings::DEFAULT_GPU_MAX_RUN_AHEAD);
        settings_changed = true;
      }
