...
bool GPU_HW_D3D11::UpdateVRAMReplacementTexture(const TextureReplacements::ReplacementTexture* tex)
{
  if (m_vram_replacement_texture.GetWidth() < tex->GetWidth() ||
      m_vram_replacement_texture.GetHeight() < tex->GetHeight())
  {
    return m_vram_replacement_texture.Create(m_device.Get(), tex->GetWidth(), tex->GetHeight(), 1,
...
bool GPU_HW_D3D11::BlitVRAMReplacementTexture(const TextureReplacements::ReplacementTexture* tex, u32 dst_x, u32 dst_y,
                                              u32 width, u32 height)
{
  if (!UpdateVRAMReplacementTexture(tex))
    return false;
...
void GPU_HW_D3D11::UpdateVRAM(u32 x, u32 y, u32 width, u32 height, const void* data, bool set_mask, bool check_mask)
{
  ...
  if (!check_mask)
  {
    const TextureReplacements::ReplacementTexture* rtex =
      g_texture_replacements.GetVRAMWriteReplacement(width, height, static_cast<const u16*>(data));
