...
ALWAYS_INLINE u16 RGBA5551ToBGRA5551(u16 value)
{
  return (value & 0x83E0) | ((value >> 10) & 0x1F) | ((value & 0x1F) << 10);
}

static void CopyOutRow15Bit(const u16* src_ptr, u16* dst_ptr, u32 width)
{
  u32 col = 0;
  ...
  for (; col < width; col++)
    *(dst_ptr++) = RGBA5551ToBGRA5551(*(src_ptr++));
}
...
void GPU_SW::CopyOut15Bit(u32 src_x, u32 src_y, u32 width, u32 height, u32 field, bool interlaced, bool interleaved)
{
  ...
  if ((src_x + width) <= VRAM_WIDTH && (src_y + height) <= VRAM_HEIGHT)
  {
    ...
    {
      CopyOutRow15Bit(src_ptr, reinterpret_cast<u16*>(dst_ptr), width);
      src_ptr += src_step;
      dst_ptr += dst_stride;
    }
  }
  else
  {
    ...
    for (u32 row = 0; row < rows; row++)
    {
      ...
      for (u32 col = src_x; col < end_x; col++)
        *(dst_row_ptr++) = RGBA5551ToBGRA5551(src_row_ptr[col % VRAM_WIDTH]);
                          ...
                          bool interleaved)
{
  ...
  if (!interlaced)
  {
    if (!m_host_display->BeginSetDisplayPixels(HostDisplay::DisplayPixelFormat::RGBA8, width, height,
                                               reinterpret_cast<void**>(&dst_ptr), &dst_stride))
    {
      return;
    }
  ...
  {
    m_host_display->SetDisplayPixels(HostDisplay::DisplayPixelFormat::RGBA8, width, height,
                                     m_display_texture_buffer.data(), sizeof(u32) * VRAM_WIDTH);
  }
}
