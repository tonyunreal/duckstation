...
void CheckAndUpdateICacheTags(u32 line_count, TickCount uncached_ticks)
{
  ...
  {
    TickCount ticks = 0;
    for (u32 i = 0; i < line_count; i++, (current_pc = (current_pc & ICACHE_TAG_ADDRESS_MASK) + ICACHE_LINE_SIZE))
    {
      if (!CompareICacheTag(current_pc))
      {
        const u32 line = GetICacheLine(current_pc);
        g_state.icache_tags[line] = GetICacheFillTagForAddress(current_pc);
        ticks += GetICacheFillTicks(current_pc);
      }
    }
...
u32 FillICache(VirtualMemoryAddress address)
{
  ...
  const u32 line_word = (address >> 2) & 0x03u;
